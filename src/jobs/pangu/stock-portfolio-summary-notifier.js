...
const getMarketDataForPortfolio = async (portfolio) => {
  ...
  const requests = portfolio.map((stock) => {
    return axios
      .get(`${STOCK_DATA_API_URL}${stock.mcStockId}`)
      .catch((error) => {
        console.error(error.toJSON());
    ...
    data
      ? {
          ...
          priceInfo: {
            ...
            close: Number(
              data.data.priceinfo.yesterdaysclose.replace(/,/g, '')
            ),
...
const calculatePortfolioSummary = (portfolioMarketData) => {
  ...
  return {
    holdings: portfolioMarketData.map((stock) => {
      const totalQuantity = stock.investments.reduce(
        (a, v) => a + v.quantity,
        0
      );
      const averagePrice =
        stock.investments.reduce(
          (a, v) => a + (v.quantity * v.price + v.charges),
          0
        ) / totalQuantity;
      const totalInvestment = averagePrice * totalQuantity;
      const totalValue = stock.priceInfo.price * totalQuantity;
      const overallGainLossValue =
        (stock.priceInfo.price - averagePrice) * totalQuantity;
      const overallGainLossPercent =
        (overallGainLossValue / totalInvestment) * 100;
    ...
    overallGainLoss: {
      ...
      percent:
        ((portfolioTotalValue - portfolioTotalInvestment) /
...
const composePortfolioSummaryTable = (portfolioSummary, niftyFiftyData) => {
  ...
  htmlTableString += portfolioSummary.holdings
    .sort(
      (a, b) =>
        b.holding.overallGainLoss.percent - a.holding.overallGainLoss.percent
    )
    .reduce((acc, val) => {
      acc += `<tr class="stock">
              <td>
                <div class="name">${val.stockName} <span>(${
        val.holding.totalQuantity
      })</span></div>
                <div class="investment">${formatNumber(
                  val.holding.averagePrice
                )} <span>(${formatNumber(
        val.holding.totalInvestment
      )})</span></div>
              </td>
              <td class="${getClass(val.priceInfo.change.direction)}">
                <div class="day-value">${formatNumber(
                  val.priceInfo.price
                )} <span>${getArrow(
        val.priceInfo.change.direction
      )}</span></div>
                <div class="day-change">${formatNumber(
                  val.priceInfo.change.value
                )} <span>(${formatNumber(
        val.priceInfo.change.percent
      )}%)</span></div>
              </td>
              <td class="${getClass(val.holding.overallGainLoss.percent)}">
                <div class="total-value">${formatNumber(
                  val.holding.totalValue
                )} <span>${getArrow(
        val.holding.overallGainLoss.percent
      )}</span></div>
                <div class="total-change">${formatNumber(
                  val.holding.overallGainLoss.value
                )} <span>(${formatNumber(
        val.holding.overallGainLoss.percent
      )}%)</span></div>
              </td>
  ...
  htmlTableString += `<tr class="portfolio">
                        ...
                        <td>
                          <div class="${getClass(
                            portfolioSummary.dayGainLoss.percent
                          )}">${formatNumber(
    portfolioSummary.dayGainLoss.value
  )} <span>(${formatNumber(
    portfolioSummary.dayGainLoss.percent
  )}%)</span> <span>${getArrow(
    portfolioSummary.dayGainLoss.percent
                          ...
                          <div class="${getClass(
                            portfolioSummary.overallGainLoss.percent
                          )}">${formatNumber(
    portfolioSummary.overallGainLoss.value
  )} <span>(${formatNumber(
    portfolioSummary.overallGainLoss.percent
  )}%)</span> <span>${getArrow(
    portfolioSummary.overallGainLoss.percent
                      ...
                      <tr class="portfolio">
                        ...
                        <td>
                          <div class="${getClass(
                            niftyFiftyData.change.percent
                          )}">${formatNumber(
    niftyFiftyData.change.percent
  )}% <span>${getArrow(niftyFiftyData.change.percent)}</span></div>
                          <div class="${getClass(
                            portfolioSummary.dayGainLoss.percent
                          )}">${formatNumber(
    portfolioSummary.dayGainLoss.percent
  )}% <span>${getArrow(portfolioSummary.dayGainLoss.percent)}</span></div>
                        </td>
                      </tr>
                    </tbody>
                  </table>`;
...
(async () => {
  ...
  if (
    !niftyFiftyData ||
    formatDate(niftyFiftyData.lastUpdated) !== formatDate(new Date())
  ) {
  ...
  if (portfolio) {
    ...
    if (portfolioMarketData) {
      ...
      const portfolioSummaryTable = composePortfolioSummaryTable(
        portfolioSummary,
        niftyFiftyData
      );
      ...
      if (imageUrl) {
        const caption = `
          NIFTY: *${formatNumber(niftyFiftyData.change.percent)
            .replace('-', '\\-')
            .replace('.', '\\.')}%*
          ${NEW_LINE}Portfolio: *${formatNumber(
          portfolioSummary.dayGainLoss.percent
        )
          .replace('-', '\\-')
          .replace('.', '\\.')}%*
